...
import { BookOpen, Brain, ChevronLeft, ChevronRight, Code2, Home, Menu, Play, RotateCcw, Settings, Sparkles, Trophy } from "lucide-react";
...
export default function HomePage() {
  ...
  const [profile, setProfile] = useState({ xp: 0, level: 1, rank: "Python Anfänger" });
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (!Array.isArray(lessons) || !lessons.length || !Array.isArray(lessons[0].steps) || !lessons[0].steps.length) {
          throw new Error("Keine gültige Lektion gefunden.");
        }
        setLesson(lessons[0]);
  ...
  return (
    <div className="app-shell">
      <aside className={`sidebar ${collapsed ? "collapsed" : ""}`}>
        ...
        <nav>
          {nav.map(([name, Icon]) => <button key={name} className={`nav-item ${activeNav === name ? "active" : ""}`} onClick={() => setActiveNav(name)} title={name}><Icon size={19}/>{!collapsed && <span>{name}</span>}</button>)}
        </nav>
      ...
      <main className="main">
        <header className="topbar">
          ...
          <div className="profile-strip"><div className="level-pill">Level {profile.level}</div><div><div className="rank">{profile.rank}</div><div className="xp">{profile.xp} XP</div></div></div>
        </header>

        {activeNav !== "Heute lernen" && activeNav !== "Lernpfad" ? (
          <section className="placeholder"><h1>{activeNav}</h1><p>Dieser Bereich wird auf der bestehenden PyLab-Lernarchitektur weiter ausgebaut.</p><button className="primary" onClick={() => setActiveNav("Heute lernen")}>Weiterlernen</button></section>
        ) : (
            ...
            <section className="lesson-card">
              ...
              <div className="lesson-content">
                ...
                {step.type === "quiz" && <div>
                  ...
                  <div className="options">{step.options.map((option, idx) => <button key={option} className={`option ${selected === idx ? "selected" : ""} ${quizChecked && idx === step.correct ? "correct" : ""} ${quizChecked && selected === idx && idx !== step.correct ? "wrong" : ""}`} onClick={() => !quizChecked && setSelected(idx)}><span>{String.fromCharCode(65 + idx)}</span><code>{option}</code></button>)}</div>
                ...
                {step.type === "code" && <div>
                  ...
                  <div className="editor-shell"><div className="editor-toolbar"><span>main.py</span><button onClick={runCode}><Play size={15}/> Ausführen</button></div><textarea className="editor" value={code} onChange={e => setCode(e.target.value)} spellCheck={false}/></div>
              ...
              <footer className="lesson-footer"><button className="ghost" disabled={stepIndex === 0} onClick={() => setStepIndex(i => Math.max(0, i - 1))}><ChevronLeft size={18}/> Zurück</button><button className="primary" disabled={!canContinue} onClick={next}>{stepIndex === lesson.steps.length - 1 ? "Lektion abschließen" : "Weiter"}<ChevronRight size={18}/></button></footer>
            </section>

            <aside className="course-panel"><span className="eyebrow">Aktuelle Lektion</span><h2>{lesson.title}</h2><p>{lesson.subtitle}</p><div className="course-meta"><span>{lesson.difficulty}</span><span>≈ {lesson.estimated_minutes} Min.</span></div><div className="step-list">{lesson.steps.map((item, idx) => <button key={item.id} className={`step-row ${idx === stepIndex ? "current" : ""} ${idx < stepIndex ? "done" : ""}`} onClick={() => idx <= stepIndex && setStepIndex(idx)}><span className="step-dot">{idx < stepIndex ? "✓" : idx + 1}</span><span>{item.title}</span></button>)}</div></aside>
          </div>
        )}
      </main>
    </div>

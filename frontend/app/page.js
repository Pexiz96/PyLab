...
export default function HomePage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setLessons(lessonData); setProfile(profileData);
        const progress = Array.isArray(profileData.progress) ? profileData.progress : [];
        let targetLesson = 0, targetStep = 0;
        for (let i=0;i<lessonData.length;i++) {
          const saved = progress.find(p => p.lesson_id === lessonData[i].id);
          if (!saved || !saved.completed) { targetLesson=i; targetStep=saved ? Math.min(saved.step_index||0, lessonData[i].steps.length-1) : 0; break; }
          if (i===lessonData.length-1) { targetLesson=i; targetStep=lessonData[i].steps.length-1; }
        }
        setLessonIndex(targetLesson); setStepIndex(targetStep);
      } catch (error) { setLoadError(error.message || "PyLab konnte nicht geladen werden."); }
  ...
  useEffect(() => {
    if (!step) return;
    setSelected(null); setQuizChecked(false); setCheckState(null); setHintIndex(0); setOutput(""); setFinishMessage("");
  ...
  function lessonProgress(id) { return profile.progress?.find(p => p.lesson_id === id) || null; }
  function lessonMastery(id) { return profile.mastery?.find(m => m.lesson_id === id) || { score:0, attempts:0, streak:0 }; }

  async function saveProgress(index, completed=false) {
    if (!lesson) return;
    const response = await fetch(`${API}/progress`, { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({lesson_id:lesson.id,step_index:index,completed}) });
  ...
  async function checkQuiz() { if (selected === null) return; setQuizChecked(true); await recordQuiz(selected === step.correct); }

  async function next() {
    if (!lesson?.steps?.length) return;
    if (stepIndex < lesson.steps.length-1) { const n=stepIndex+1; setStepIndex(n); await saveProgress(n,false); return; }
    await saveProgress(stepIndex,true);
    if (lessonIndex < lessons.length-1) { const n=lessonIndex+1; setLessonIndex(n); setStepIndex(0); setFinishMessage(`Lektion abgeschlossen. Weiter mit: ${lessons[n].title}`); }
    else { setFinishMessage("Alle aktuell verfügbaren Lektionen sind abgeschlossen."); setActiveNav("Lernpfad"); }
  }

  async function goBack() { if (stepIndex<=0) return; const p=stepIndex-1; setStepIndex(p); await saveProgress(p,false); }
  function openLesson(index) { const target=lessons[index]; const saved=lessonProgress(target.id); setLessonIndex(index); setStepIndex(saved ? Math.min(saved.step_index||0,target.steps.length-1) : 0); setActiveNav("Heute lernen"); }
  function openReview(review) { const index=lessons.findIndex(l=>l.id===review.lesson_id); if(index>=0){ setLessonIndex(index); setStepIndex(0); setActiveNav("Heute lernen"); } }

  async function runCode() {
    const response=await fetch(`${API}/run`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({code})});
    const data=await response.json(); setOutput(data.stderr||data.stdout||"(keine Ausgabe)");
  }

  async function checkCode() {
    const response=await fetch(`${API}/check`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({code,expected_output:step.expected_output,lesson_id:lesson.id,step_id:step.id,xp:step.xp||40})});
    const data=await response.json(); setOutput(data.stderr||data.stdout||"(keine Ausgabe)"); setCheckState(data.passed?"success":"error"); await refreshProfile();
  }
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      <header className="topbar"><div><div className="top-title">{activeNav}</div><div className="top-subtitle">Learn. Code. Master Python.</div></div><div className="profile-strip"><div className="mastery-mini"><Target size={15}/>{profile.average_mastery||0}%</div><div className="level-pill">Level {profile.level}</div><div><div className="rank">{profile.rank}</div><div className="xp">{profile.xp} XP</div></div></div></header>

      {activeNav==="Lernpfad" ? <section className="path-page">
        <div className="path-heading"><span className="eyebrow">Python Lernpfad</span><h1>Schritt für Schritt Python beherrschen</h1><p>Fortschritt zeigt, was du erledigt hast. Mastery zeigt, was wirklich sitzt.</p></div>
      ...
      : <div className="lesson-layout"><section className="lesson-card">{finishMessage&&<div className="finish-banner">{finishMessage}</div>}
        ...
        <footer className="lesson-footer"><button className="ghost" disabled={stepIndex===0} onClick={goBack}><ChevronLeft size={18}/> Zurück</button><button className="primary" disabled={!canContinue} onClick={next}>{stepIndex===lesson.steps.length-1?"Lektion abschließen":"Weiter"}<ChevronRight size={18}/></button></footer>
      </section>
      <aside className="course-panel"><span className="eyebrow">Dein Lernstand</span><h2>{lesson.title}</h2><div className="mastery-ring"><strong>{currentMastery.score}%</strong><span>{masteryLabel(currentMastery.score)}</span></div><div className="course-meta"><span>{currentMastery.attempts} Versuche</span><span>Serie {currentMastery.streak}</span><span>≈ {lesson.estimated_minutes} Min.</span></div><button className={`visual-toggle ${visualMode?"on":""}`} onClick={()=>setVisualMode(v=>!v)}><Eye size={16}/> Visuelle Erklärung {visualMode?"an":"aus"}</button><div className="step-list">{lesson.steps.map((s,i)=><button key={s.id} className={`step-row ${i===stepIndex?"current":""} ${i<stepIndex?"done":""}`} onClick={()=>setStepIndex(i)}><span className="step-dot">{i<stepIndex?<Check size={13}/>:i+1}</span><span>{s.title}</span></button>)}</div></aside></div>}
    </main>
  </div>;
}
